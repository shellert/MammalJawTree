...
#setwd("/Users/eargtl")
setwd("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/mrp2")
...
for(i in 1:length(trees.list)) {
  ...
  if(length(mytrees$mpts) == 100000 || sum(mrp.list == gsub("tntmpts_plus_strict.nex", "mrp.nex", trees.list[i])) == 0) {
    ...
    mymatrix <- read_nexus_matrix(paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/nexus2", "/", nexus.filename, sep = ""))
  ...
  # Write out MPTs:
  write(mytrees$mpts, paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/mpts", "/", file.name, ".tre", sep = ""))
  
  # Write out first MPT:
  write(mytrees$mpts[1], paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/firstmpt", "/", file.name, ".tre", sep = ""))
  
  # Write out strict consensus:
  write(mytrees$strict, paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/sc", "/", file.name, ".tre", sep = ""))
...
for(i in 1:length(mrp.list)) {
  ...
  # Isolate full names:
  nexus.names <- rownames(read_nexus_matrix(paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/nexus2/", gsub("mrp", "", file.name), ".nex", sep = ""))$matrix_1$matrix)
  ...
  write_nexus_matrix(mymrp, paste("~/Desktop/Desktop_Spencers_MacBook_Pro_2/Grossnickle Tree/MRP", "/", file.name, ".nex", sep = ""))
